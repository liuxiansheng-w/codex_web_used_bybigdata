...
import { visibleActivity, activityNotification } from './activity-items.mjs';
...
function visibleItem(item, context) {
  ...
  if (['reasoning', 'commandExecution', 'mcpToolCall', 'dynamicToolCall', 'webSearch', 'imageView'].includes(item.type)) return visibleActivity(item);
...
export class Workspace extends EventEmitter {
  ...
  importThread(raw, extra = {}, history = {}) {
    ...
    const items = (raw.turns || []).flatMap(turn => {
      ...
      return visible.map(item => {
        ...
        if (old?.progress) item.progress = old.progress;
        if (old?.output) item.output = old.output;
    ...
    // Plan notifications aren't persisted as native items. Keep observed plans
    // on read refreshes, without pretending they can be recovered after restart.
    for (const old of previous?.items || []) if (old.type === 'activityPlan' && raw.turns?.some(turn => turn.id === old.turnId)) {
      const first = items.findIndex(item => item.turnId === old.turnId && item.type !== 'userMessage');
      items.splice(first < 0 ? items.length : first, 0, old);
    }
  ...
  notification({ method, params: p = {} }) {
    ...
    if (method === 'item/autoApprovalReview/started' || method === 'item/autoApprovalReview/completed') {
      ...
      if (!item) {
        const summary = p.item.type === 'reasoning' && t.items.find(item => item.id === p.item.id && item.type === 'activitySummary');
        if (summary && method === 'item/completed') { summary.status = 'completed'; this.changed(t); }
        return;
      }
      if (['webSearch', 'imageView', 'activitySummary'].includes(item.type)) item.status = method.endsWith('/started') ? 'inProgress' : 'completed';
      ...
      if (previous?.progress) item.progress = previous.progress;
      if (previous?.output) item.output = previous.output;
      ...
      if (item) { const output = (item.text || '') + p.delta; item.outputTruncated ||= output.length > 100_000; item.text = output.slice(-100_000); }
    } else if (activityNotification(t, method, p)) {
      // A display-only notification; no tool or model request is made here.
